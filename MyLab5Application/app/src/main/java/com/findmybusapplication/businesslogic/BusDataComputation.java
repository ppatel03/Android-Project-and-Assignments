...
class BusArrivalTimeComparator implements Comparator<Bus>{
    ...
    public int compare(Bus b1, Bus b2) {
        // TODO Auto-generated method stub
        Double difference = (b1.getArrivalTimeAtStop() - b2.getArrivalTimeAtStop());
        if(difference < 0){
            return -1;
        }
        else if (difference == 0){
...
public class BusDataComputation {
	...
	public void sortAccordingToCriteria(String sortType, List<Bus> selectedBuses){
		if(sortType != null){
			...
			case CriteriaConstants.TOTAL_TRIP_TIME_CRITERIA : Collections.sort(selectedBuses, new BusTripTimeComparator()); break;
            case CriteriaConstants.TIME_CRITERIA : Collections.sort(selectedBuses, new BusArrivalTimeComparator()); break;
			}
		}
